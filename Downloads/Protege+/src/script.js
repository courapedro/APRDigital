...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===================================================================
    // FUNÇÃO PARA SALVAR FORMULÁRIO E ATUALIZAR JSON NO GITHUB
    // ===================================================================
    window.salvarFormularioFinalizado = async function() {
        try {
            const step1Data = window.getAprStep1Data() || {};
            const step2Data = window.getAprStep2Data() || {};
            const step3Data = window.getAprStep3Data() || {};
            ...
            const formularioId = window.generateFormId();
            const agora = new Date();

            // Constrói o formulário com ID
            const formulario = {
                id: formularioId,
            ...
            // Recupera formulários já salvos do localStorage
            let todosFormularios = [];
            const jsonSalvo = localStorage.getItem('aprFormularios');
            if (jsonSalvo) {
                todosFormularios = JSON.parse(jsonSalvo);
            }

            // Adiciona o novo formulário
            todosFormularios.push(formulario);

            // Salva em localStorage
            localStorage.setItem('aprFormularios', JSON.stringify(todosFormularios));

            // Tenta atualizar no GitHub (se configurado)
            await window.atualizarGitHub(todosFormularios);

            console.log('✅ Formulário salvo com ID:', formularioId);
            console.log('📊 Total de formulários:', todosFormularios.length);

            return { sucesso: true, id: formularioId, total: todosFormularios.length };
    ...
    // ===================================================================
    window.atualizarGitHub = async function(formularios) {
        try {
            // Configurações (você precisa definir essas variáveis)
            const GITHUB_TOKEN = localStorage.getItem('githubToken');
            const GITHUB_REPO = localStorage.getItem('githubRepo'); // formato: usuario/repo
            const GITHUB_BRANCH = 'main';
            const ARQUIVO_PATH = 'formularios.json';

            if (!GITHUB_TOKEN || !GITHUB_REPO) {
                console.warn('⚠️ GitHub não configurado. Dados salvos apenas localmente.');
                return;
            }

            // Prepara o conteúdo do arquivo
            const conteudo = JSON.stringify(formularios, null, 2);
            const contentBase64 = btoa(unescape(encodeURIComponent(conteudo)));

            // Obtém informações do arquivo atual (para obter o SHA)
            const urlGet = `https://api.github.com/repos/${GITHUB_REPO}/contents/${ARQUIVO_PATH}?ref=${GITHUB_BRANCH}`;
            ...
            const urlPut = `https://api.github.com/repos/${GITHUB_REPO}/contents/${ARQUIVO_PATH}`;
            ...
            const responsePut = await fetch(urlPut, {
                ...
                body: JSON.stringify({
                    message: `Novo formulário salvo: ${formularios[formularios.length - 1].id}`,
            ...
            if (responsePut.ok) {
                ...
                const erro = await responsePut.json();
                console.warn('⚠️ Erro ao atualizar GitHub:', erro.message);
                return false;
            }

        } catch (error) {
            console.warn('⚠️ Erro ao conectar ao GitHub:', error.message);
            return false;
        }
    };

    // ===================================================================
    // FUNÇÃO PARA CONFIGURAR TOKEN DO GITHUB
    // ===================================================================
    window.configurarGitHub = function() {
        const token = prompt('Digite seu GitHub Personal Access Token (Fine-grained):\n\nVocê precisa criar em: https://github.com/settings/tokens?type=beta\nPermissões necessárias: Contents (read/write)');
        if (token) {
            localStorage.setItem('githubToken', token);
            alert('✅ Token salvo!');
        }

        const repo = prompt('Digite seu repositório (formato: usuario/repo)\nExemplo: seu-usuario/seu-repo');
        if (repo) {
    ...
    // ===================================================================
    // FUNÇÃO PARA VISUALIZAR FORMULÁRIOS SALVOS
    // ===================================================================
    window.visualizarFormularios = function() {
        try {
            const jsonSalvo = localStorage.getItem('aprFormularios');
            const formularios = jsonSalvo ? JSON.parse(jsonSalvo) : [];

            if (formularios.length === 0) {
                alert('Nenhum formulário salvo ainda.');
                return;
            }

            let resumo = `FORMULÁRIOS SALVOS (Total: ${formularios.length})\n\n`;
            formularios.forEach((form, index) => {
                resumo += `${index + 1}. ID: ${form.id}\n`;
                resumo += `   Data: ${form.data}\n`;
                resumo += `   Responsável: ${form.step1.responsavel || 'N/A'}\n`;
                resumo += `   Descrição: ${form.step1.descricao || 'N/A'}\n\n`;
            });
    ...
    // ===================================================================
    // FUNÇÃO PARA EXPORTAR TODOS OS FORMULÁRIOS EM JSON
    // ===================================================================
    window.exportarFormularios = function() {
        try {
            const jsonSalvo = localStorage.getItem('aprFormularios');
            const formularios = jsonSalvo ? JSON.parse(jsonSalvo) : [];
            ...
            const jsonContent = JSON.stringify(formularios, null, 2);
            ...
            console.log('✅ Arquivo formularios.json exportado com', formularios.length, 'formulários');
